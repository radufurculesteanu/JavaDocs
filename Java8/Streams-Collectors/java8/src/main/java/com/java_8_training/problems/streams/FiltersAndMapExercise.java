...
public class FiltersAndMapExercise {
    ...
    public static List<String> findPersonByName(String name){
        ...
        personName = input.filter(value -> value.equals("Jack")).collect(toList());
        return personName;
    }
    ...
    public static List<Person> findPersonByAgeAndName(List<Person> personList, String name, Integer age){
        ...
        persons = personStream.filter(value -> value.getName().equals("James") && value.getAge()==20).collect(toList());
        return persons;
    }
    ...
    public static List<String> getOnlyNames(List<Person> personList){
        ...
        // HINT: use map
        Stream<Person> personStream = personList.stream();
        collect = personStream.map(value -> value.getName()).collect(toList());
        return  collect;
    }
    ...
    public static int product(){
        ...
        int product = 1;
        //TODO #8
        product = input.reduce(1,(value,acc) -> value * acc);
        return product;
    }
    ...
    public static Optional<Person> findFirstAllMalePersons(List<Person> personList){
        ...
        //TODO #9
        Stream <Person> personStream = personList.stream();
        personOptional = personStream.filter(value -> value.isMale()).findFirst();
    ...
    public static boolean areAllPersonsBelowAge(List<Person> personList, Integer age){
        ...
        Stream<Person> personStream = personList.stream();
        overAge = personStream.allMatch(p -> p.getAge() < 70);
        return overAge;
    }
    ...
    public static int findTheYoungestPerson(List<Person> personList){
        ...
        min = personStream.map(person -> person.getAge()).reduce(Integer.MAX_VALUE,(acc,x) -> Integer.min(acc,x));
    ...
    public static double findAgeAverage(List<Person> personList){
        ...
        //TODO #13
        Stream<Person> personStream = personList.stream();
        sum = personStream.map(person -> person.getAge()).reduce(0,(acc,x) -> acc+x);
    ...
    public static List<Nationality> findNationalityByName(List<Person> personList){
        ...
        Stream<Person> personStream = personList.stream();
        persons = personStream.filter(person -> person.getName().startsWith("J")).map(person -> person.getNationality()).distinct().collect(toList());
    ...
    public static Optional<Integer> findSquareDivisibleBy3(){
        ...
        Stream<Integer> numberStream = someNumbers.stream();
        firstSquareDivisibleByThree = numberStream.filter(value -> Math.sqrt(value)%3==0).findFirst();
    ...
    public static int sumStringLength(){
        ...
        sum = stringsStream.map(word -> word.length()).reduce(0,(acc,x) -> acc+x);
        return sum;
    }
    ...
    public static  int sumSalary(List<Person> personList){
        ...
        //TODO #17
        Stream<Person> personStream = personList.stream();
        sum = personStream.map(person -> person.getSalary()).reduce(0,(acc,x) -> acc+x);
        return sum;
    }
